...
public class GravityManipulator : MonoBehaviour {
    ...
    public virtual void ApplyGravity(Transform obj) {
        if (enableGravityFallOff) {
            float _distance = Vector3.Distance(transform.position, obj.position);

            float _attenuation = 1f / Mathf.Clamp((_distance * _distance), 1f, Mathf.Infinity);
            GravityForce *= _attenuation;
        }
    ...
    private void SphericalGravity(Transform obj) {
        ...
        Vector3 _objectUpDirection = obj.up;
        // Direction pointing from gravity source to object
        Vector3 _gravityUpDirection = (obj.position - transform.position).normalized;
        // Rotation to align object 'up' with gravity direction
        Quaternion _targetRotation = Quaternion.FromToRotation(_objectUpDirection, _gravityUpDirection) * obj.rotation;

        switch (gameDimension) {
            case GameDimension.TwoDimensional:
                // Adjust rotation only on the Z-axis for 2D
                _targetRotation = Quaternion.Euler(0, 0, _targetRotation.z);
                // Smoothly interpolate object's rotation to align with gravity direction
                obj.rotation = Quaternion.Lerp(obj.rotation, _targetRotation, rotationalCorrectionSpeed * Time.deltaTime);

                // Apply force toward gravity source
                obj.GetComponent<Rigidbody2D>().AddForce(_gravityUpDirection * GravityForce);
                break;
            case GameDimension.ThreeDimensional:
                // Smoothly interpolate object's rotation to align with gravity direction
                obj.rotation = Quaternion.Slerp(obj.rotation, _targetRotation, rotationalCorrectionSpeed * Time.deltaTime);

                // Apply force toward gravity source
                obj.GetComponent<Rigidbody>().AddForce(_gravityUpDirection * GravityForce);
                break;
        }
    }
    /// <summary>
    ...
    private void DirectionalGravity(Transform Obj) {
        ...
        Vector3 _gravityUpDirection = Vector3.zero;
        ...
        if (bidirectionalGravity) {
            ...
            switch (axis) {
                case Axis.X_Axis: _gravityUpDirection = new Vector3(direction.x, 0, 0).normalized; break;
                case Axis.Y_Axis: _gravityUpDirection = new Vector3(0, direction.y, 0).normalized; break;
                case Axis.Z_Axis: _gravityUpDirection = new Vector3(0, 0, direction.z).normalized; break;
            }
        }
        else {
            switch (axis) {
                case Axis.X_Axis: _gravityUpDirection = Vector3.right; break;
                case Axis.Y_Axis: _gravityUpDirection = Vector3.up; break;
                case Axis.Z_Axis: _gravityUpDirection = Vector3.forward; break;
            }
        }

        // Rotation to align object 'up' with gravity direction
        Quaternion TargetRotation = Quaternion.FromToRotation(_objectUpDirection, _gravityUpDirection) * Obj.rotation;
        // Smoothly interpolate object's rotation to align with gravity direction
    ...
    protected virtual void IntervalCheckForGravityBodies() {
        ...
        // Find all gravity body in the scene
        GravityBody[] _allGravityBodies = FindObjectsByType<GravityBody>(FindObjectsSortMode.None);
        // Register this manipulator with the gravity body
        foreach (GravityBody _objBody in _allGravityBodies) {
            if (_objBody != null) {
                _objBody.AddGravityManipulator(this);
            }
        }
    ...
    private void OnDestroy() {
        ...
        foreach (GravityBody _objBody in _allGravityBodies) {
            if (_objBody != null) {
                _objBody.RemoveGravityManipulator(this);
            }
        }
    }
    #endregion
